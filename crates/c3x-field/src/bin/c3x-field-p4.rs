...
#[derive(Parser)]
#[command(name="c3x-field-p4",about="Independent P4 causal-field assessment/query kernel")]
...
fn key(fiber:&str,ctx:&Value,cs:&[String])->String{
 let mut p=vec![fiber.to_string()];
 for c in cs{p.push(format!("{}={}",c,ctx[c].as_str().unwrap()));}
 p.join("||")
}
fn assess(input:PathBuf,schemas:PathBuf,out:PathBuf){
 let x=load(&input);let ss=load(&schemas);assert_eq!(x["scientific_stage"],STAGE);
 let rows=x["records"].as_array().unwrap();let mut census=Vec::new();
 for s in ss.as_array().unwrap(){
  ...
  let mut col=0usize;let mut xp=0usize;let mut xe=0usize;
  for z in g.values(){
 ...
 save(&out,&json!({"schema":"c3x-field-p4-assessment-v1","scientific_stage":STAGE,"census":census}));
}
fn query(field:PathBuf,profiles:PathBuf,out:PathBuf){
 ...
 for r in p["records"].as_array().unwrap(){
  ...
  if let Some(c)=cells.get(&k){
   let lab=c["label"].as_str().unwrap();
   z.push(json!({"record_id":r["record_id"],"engine":r["engine"],"position_id":r["position_id"],"source_stratum":r["source_stratum"],
    "cell_key":k,"status":if lab=="ROOT_CHANGE"{"CERTIFIED_ROOT_CHANGE"}else{"CERTIFIED_NO_ROOT_CHANGE"},
    "predicted_root_change":lab=="ROOT_CHANGE","discovery_support":c["support"]}));
  }else{
   z.push(json!({"record_id":r["record_id"],"engine":r["engine"],"position_id":r["position_id"],"source_stratum":r["source_stratum"],
    "cell_key":k,"status":"ABSTAIN_UNSEEN_CONTEXT","predicted_root_change":Value::Null,"discovery_support":0}));
  }
 }
 save(&out,&json!({"schema":"c3x-field-p4-query-v1","scientific_stage":STAGE,"predictions":z,"target_fields_consulted":false}));
}
fn diagnose(predictions:PathBuf,targets:PathBuf,out:PathBuf){
 let p=load(&predictions);let t=load(&targets);
 let tm:BTreeMap<_,_>=t["records"].as_array().unwrap().iter().map(|r|(r["record_id"].as_str().unwrap(),r)).collect();
 ...
 for r in p["predictions"].as_array().unwrap(){
  ...
  if q!=y{bad.push(json!({"record_id":r["record_id"],"engine":r["engine"],"position_id":r["position_id"],"source_stratum":r["source_stratum"],
    "cell_key":r["cell_key"],"predicted_root_change":q,"observed_root_change":y}));}
 }
 save(&out,&json!({"schema":"c3x-field-p4-diagnosis-v1","scientific_stage":STAGE,"authority":"POST_TARGET_DIAGNOSTIC_ONLY",
  "may_modify_field":false,"abstained":abst,"contradictions":bad}));
}
fn main(){let c=Cli::parse();match c.cmd{
 Cmd::Assess{input,schemas,out}=>assess(input,schemas,out),
 Cmd::Query{field,profiles,out}=>query(field,profiles,out),
