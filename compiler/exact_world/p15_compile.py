...
VERSION="c3x-p15-stage-a-v1"
OFFSET=20000

MATERIALS=[
 ("KQvKBB",[(chess.WHITE,chess.KING),(chess.WHITE,chess.QUEEN),(chess.BLACK,chess.KING),(chess.BLACK,chess.BISHOP),(chess.BLACK,chess.BISHOP)]),
 ("KQvKNN",[(chess.WHITE,chess.KING),(chess.WHITE,chess.QUEEN),(chess.BLACK,chess.KING),(chess.BLACK,chess.KNIGHT),(chess.BLACK,chess.KNIGHT)]),
 ("KRRvKB",[(chess.WHITE,chess.KING),(chess.WHITE,chess.ROOK),(chess.WHITE,chess.ROOK),(chess.BLACK,chess.KING),(chess.BLACK,chess.BISHOP)]),
 ("KRRvKN",[(chess.WHITE,chess.KING),(chess.WHITE,chess.ROOK),(chess.WHITE,chess.ROOK),(chess.BLACK,chess.KING),(chess.BLACK,chess.KNIGHT)]),
 ("KRBvKB",[(chess.WHITE,chess.KING),(chess.WHITE,chess.ROOK),(chess.WHITE,chess.BISHOP),(chess.BLACK,chess.KING),(chess.BLACK,chess.BISHOP)]),
 ("KRBvKN",[(chess.WHITE,chess.KING),(chess.WHITE,chess.ROOK),(chess.WHITE,chess.BISHOP),(chess.BLACK,chess.KING),(chess.BLACK,chess.KNIGHT)]),
 ("KRNvKB",[(chess.WHITE,chess.KING),(chess.WHITE,chess.ROOK),(chess.WHITE,chess.KNIGHT),(chess.BLACK,chess.KING),(chess.BLACK,chess.BISHOP)]),
 ("KRNvKN",[(chess.WHITE,chess.KING),(chess.WHITE,chess.ROOK),(chess.WHITE,chess.KNIGHT),(chess.BLACK,chess.KING),(chess.BLACK,chess.KNIGHT)]),
]
...
def query(fen,retries=5):
 ...
 for i in range(retries):
  try:
   req=urllib.request.Request(url,headers={"User-Agent":"C3X-P15/1.0 selective-factorial"})
...
def world_receipt(b):
 obj,raw_sha,url=query(b.fen()); c2w={"win":2,"draw":0,"loss":-2}
 ...
 for x in obj.get("moves",[]):
  ...
  sw=c2w[cat]
  moves.append({"uci":x["uci"],"mover_wdl":-sw,"successor_category":cat,"successor_precise_dtz":int(pd),"mover_precise_dtz":-int(pd),"zeroing":bool(x.get("zeroing",False))})
 if len(moves)<2:return None
 vals=[m["mover_wdl"] for m in moves];best=max(vals)
 if all(v==best for v in vals):return None
 counts=Counter(vals)
 return {"provider":"lichess-syzygy-http","endpoint":API,"query_url":url,"root_category":obj.get("category"),"root_wdl":c2w[obj["category"]],"root_precise_dtz":obj.get("precise_dtz"),"moves":sorted(moves,key=lambda z:z["uci"]),"world_pattern":{str(k):counts[k] for k in sorted(counts)},"optimal_count":sum(v==best for v in vals),"strictly_worse_count":sum(v<best for v in vals),"raw_response_sha256":raw_sha}
...
def tau4(board,max_paths=350000):
 ...
 for (key,lh),paths in endpoints.items():
  ...
  if len(u)>=2:
   dup+=1;tau+=len(u)-1
   if len(w)<3:w.append({"endpoint_state":key,"legal_move_set_sha256":lh,"paths":u[:4]})
 return {"tau4":tau,"duplicate_endpoint_states":dup,"enumerated_depth4_paths":path_count,"excluded_history_paths":excluded,"overflow":False,"witnesses":w}
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument("--out",required=True);ap.add_argument("--target-count",type=int,default=40);ap.add_argument("--max-generated",type=int,default=640);a=ap.parse_args()
 ...
 for m,p,s,i in schedule:
  ...
  base={"compiler_version":VERSION,"generation_index":OFFSET+i,"material_seed_name":m,"material_signature":sig(b),"side_to_move":"WHITE" if b.turn else "BLACK","fen":b.fen(),"legal_move_count":b.legal_moves.count(),"world":w,"tau4":t}
 ...
 payload={"schema":"c3x-p15-stage-a-pool-v1","scientific_stage":"C3X 0.7.0-G9.4-P15","compiler_version":VERSION,"stockfish_outcomes_consulted":False,"freshness":{"p13_p14_material_family_overlap":False,"generation_index_offset":OFFSET},"world_authority":{"provider":"Lichess public Syzygy tablebase API","endpoint":API,"robust_categories_only":["win","draw","loss"],"precise_dtz_required_for_every_move":True,"root_halfmove_clock":0,"pawnless":True},"audit_counts":{"generated_schedule":len(schedule),"legal_nonterminal_audited":audited,"world_split_pass":world_pass,"stage_a_accepted":len(accepted)},"candidates":accepted}
