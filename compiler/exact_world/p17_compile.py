...
import chess
API="https://tablebase.lichess.ovh/standard";VERSION="c3x-p17-stage-a-v1";OFFSET=40000
MATERIALS=[
...
("KQBvKB",[(chess.WHITE,chess.KING),(chess.WHITE,chess.QUEEN),(chess.WHITE,chess.BISHOP),(chess.BLACK,chess.KING),(chess.BLACK,chess.BISHOP)]),
("KQBvKN",[(chess.WHITE,chess.KING),(chess.WHITE,chess.QUEEN),(chess.WHITE,chess.BISHOP),(chess.BLACK,chess.KING),(chess.BLACK,chess.KNIGHT)]),
("KQNvKB",[(chess.WHITE,chess.KING),(chess.WHITE,chess.QUEEN),(chess.WHITE,chess.KNIGHT),(chess.BLACK,chess.KING),(chess.BLACK,chess.BISHOP)]),
("KQNvKN",[(chess.WHITE,chess.KING),(chess.WHITE,chess.QUEEN),(chess.WHITE,chess.KNIGHT),(chess.BLACK,chess.KING),(chess.BLACK,chess.KNIGHT)])]
...
def query(fen,retries=5):
 ...
 for i in range(retries):
  try:
   req=urllib.request.Request(url,headers={"User-Agent":"C3X-P17/1.0 dynamic-phase"})
...
def sig(b):
 ...
 return "_".join(out)
def main():
 ap=argparse.ArgumentParser();ap.add_argument("--out",required=True);ap.add_argument("--target-count",type=int,default=72);ap.add_argument("--max-generated",type=int,default=1200);a=ap.parse_args()
 ...
 payload={"schema":"c3x-p17-stage-a-pool-v1","scientific_stage":"C3X 0.7.0-G9.4-P17","compiler_version":VERSION,"stockfish_outcomes_consulted":False,"freshness":{"p13_p14_p15_material_family_overlap":False,"generation_index_offset":OFFSET},"world_authority":{"provider":"Lichess public Syzygy tablebase API","endpoint":API,"robust_categories_only":["win","draw","loss"],"precise_dtz_required_for_every_move":True,"root_halfmove_clock":0,"pawnless":True},"audit_counts":{"generated_schedule":len(sched),"legal_nonterminal_audited":aud,"world_split_pass":wp,"stage_a_accepted":len(acc)},"candidates":acc};payload["pool_sha256"]=h(canon(payload))
