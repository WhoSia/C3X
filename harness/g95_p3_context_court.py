...
def select_field(a):
 ...
 out={"schema":"c3x-field-p3-discovery-v1","scientific_stage":STAGE,"status":status,"support":{"records":len(rs),"positive_records":sum(r["target"] for r in rs),"negative_records":sum(not r["target"] for r in rs),"pass":support},"schema_census":census,"development_rival_census":rival_census,"selected_global_schema":sel,"engine_specific":eng,"scope_gate":law["heldout_firewall"]["pre_target_scope_gate"],"engine_scope_gate":law["heldout_firewall"]["engine_specific_scope_gate"],"target_fields_consulted":True,"primary_selection_consulted_development_rivals":False}
