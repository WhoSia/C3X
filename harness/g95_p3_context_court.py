...
def case_run(a):
 ...
 for j,e in enumerate(cand):
  ...
  profiles.append({"record_id":rid,"engine":case["engine"],"position_id":case["position_id"],"event_id":e["address_id"],"fiber_id":fid,"context":cv,"architecture_context":av})
 ...
 pd={"schema":"c3x-context-profile-batch-v2","schema_version":"c3x-context-v2","scientific_stage":STAGE,"case_id":case["case_id"],"role":case["role"],"records":profiles,"target_fields_consulted":False,"raw_full_key_emitted":False}
...
def select_field(a):
 ...
 fam=law["schema_families"];rc={z["schema_id"]:z for z in rust["census"]};census=[];good=[]
 for f in fam:
 ...
 out={"schema":"c3x-field-p3-discovery-v1","scientific_stage":STAGE,"status":status,"support":{"records":len(rs),"positive_records":sum(r["target"] for r in rs),"negative_records":sum(not r["target"] for r in rs),"pass":support},"schema_census":census,"selected_global_schema":sel,"engine_specific":eng,"scope_gate":law["heldout_firewall"]["pre_target_scope_gate"],"engine_scope_gate":law["heldout_firewall"]["engine_specific_scope_gate"],"target_fields_consulted":True}
...
def merge_profiles(a):
 ...
 rows=sorted([r for b in bs for r in b["records"]],key=lambda z:z["record_id"]);Path(a.out).write_text(json.dumps({"schema":"c3x-context-profile-batch-v2","schema_version":"c3x-context-v2","scientific_stage":STAGE,"records":rows,"target_fields_consulted":False,"raw_full_key_emitted":False},indent=2,sort_keys=True)+"\n");print("P3_PROFILES",len(rows))
...
def scope_run(a):
 ...
 out={"schema":"c3x-field-p3-scope-v1","scientific_stage":STAGE,"selected_global_schema_id":None if not f["selected_global_schema"] else f["selected_global_schema"]["id"],"global_predictions":gp,"global_coverage":gc,"engine_specific":eng,"target_fields_consulted":False}
...
def verify_run(a):
 ...
 verdict="TOPOLOGY_FIELD_SCOPE_INSUFFICIENT_HOLD" if not gpass else "TOPOLOGY_CONTEXT_CAUSAL_FIELD_HELDOUT_CERTIFIED" if gok else "ENGINE_CONDITIONAL_FIELDS_SUPPORTED_CROSS_ENGINE_GLUE_HOLD" if allok else "TOPOLOGY_FIELD_HELDOUT_FALSIFIED"
